...
@interface UIView ()
@property (nonatomic, strong, readonly) IFIBToastView *ybib_toast;
@end
...
- (void)ybib_showToastWithText:(NSString *)text type:(YBIBToastType)type hideAfterDelay:(NSTimeInterval)delay {
    ...
    IFIBToastView *toast = self.ybib_toast;
    if (!toast.superview) {
...
- (void)setYbib_toast:(IFIBToastView *)ybib_toast {
    objc_setAssociatedObject(self, YBIBToastKey, ybib_toast, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (IFIBToastView *)ybib_toast {
    IFIBToastView *toast = objc_getAssociatedObject(self, YBIBToastKey);
    if (!toast) {
        toast = [IFIBToastView new];
