...
//  IFImageBrowserKit
//
//  Created by MrGLZh on 2018/8/29.
...
@implementation UIImage (IFImageBrowser)
...
NSBundle *YBIBDefaultBundle(void) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSBundle *_bundle = [NSBundle bundleForClass:NSClassFromString(@"IFImageBrowserKit")];
        NSString *path = [_bundle pathForResource:@"IFImageBrowserKit" ofType:@"bundle"];
...
@implementation IFIBIconManager

+ (instancetype)sharedManager {
    static IFIBIconManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [IFIBIconManager new];
    });
    return manager;
}
