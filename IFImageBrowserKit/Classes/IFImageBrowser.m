...
@interface IFImageBrowser () <UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, strong) IFIBCollectionView *collectionView;
@property (nonatomic, strong) IFIBDataMediator *dataMediator;
@property (nonatomic, strong) IFIBScreenRotationHandler *rotationHandler;
@end

@implementation IFImageBrowser {
...
- (void)initValue {
    ...
    _defaultAnimatedTransition = _animatedTransition = [IFIBAnimatedTransition new];
    _toolViewHandlers = @[[IFIBToolViewHandler new]];
...
- (void)buildToolView {
    for (id<IFIBToolViewHandler> handler in self.toolViewHandlers) {
        ...
        if ([handler respondsToSelector:@selector(setYb_currentData:)]) {
            [handler setYb_currentData:^id<IFIBDataProtocol>{
...
- (void)pageNumberChanged {
    id<IFIBDataProtocol> data = self.currentData;
    ...
    for (UICollectionViewCell<IFIBCellProtocol> *cell in visibleCells) {
...
- (void)showToView:(UIView *)view containerSize:(CGSize)containerSize {
    ...
    id<IFIBDataProtocol> data = [self.dataMediator dataForCellAtIndex:self.currentPage];
    if ([data respondsToSelector:@selector(yb_projectiveView)]) {
...
- (void)hide {
    ...
    UICollectionViewCell<IFIBCellProtocol> *cell = (UICollectionViewCell<IFIBCellProtocol> *)self.collectionView.centerCell;
...
- (id<IFIBDataProtocol>)currentData {
...
- (void)implementOperateBrowserProtocol:(id<IFIBOperateBrowserProtocol>)obj {
    __weak typeof(self) wSelf = self;
...
- (void)implementGetBaseInfoProtocol:(id<IFIBGetBaseInfoProtocol>)obj {
    ...
    if ([obj respondsToSelector:@selector(setYb_auxiliaryViewHandler:)]) {
        [obj setYb_auxiliaryViewHandler:^id<IFIBAuxiliaryViewHandler>{
            __strong typeof(wSelf) self = wSelf;
            if (!self) return nil;
    ...
    if ([obj respondsToSelector:@selector(setYb_webImageMediator:)]) {
        [obj setYb_webImageMediator:^id<IFIBWebImageMediator> {
            __strong typeof(wSelf) self = wSelf;
            if (!self) return nil;
...
- (UICollectionViewCell *)collectionView:(IFIBCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    id<IFIBDataProtocol> data = [self.dataMediator dataForCellAtIndex:indexPath.row];
    
    UICollectionViewCell<IFIBCellProtocol> *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[collectionView reuseIdentifierForCellClass:data.yb_classOfCell] forIndexPath:indexPath];
...
- (IFIBContainerView *)containerView {
    if (!_containerView) {
        _containerView = [IFIBContainerView new];
...
- (IFIBCollectionView *)collectionView {
    if (!_collectionView) {
        _collectionView = [IFIBCollectionView new];
...
- (IFIBDataMediator *)dataMediator {
    if (!_dataMediator) {
        _dataMediator = [[IFIBDataMediator alloc] initWithBrowser:self];
...
- (IFIBScreenRotationHandler *)rotationHandler {
    if (!_rotationHandler) {
        _rotationHandler = [[IFIBScreenRotationHandler alloc] initWithBrowser:self];
    }
