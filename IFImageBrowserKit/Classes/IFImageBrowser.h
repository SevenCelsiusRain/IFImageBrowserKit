...
@interface IFImageBrowser : UIView

/// 数据源数组
@property (nonatomic, copy) NSArray<id<IFIBDataProtocol>> *dataSourceArray;

/// 数据源代理
@property (nonatomic, weak) id<IFImageBrowserDataSource> dataSource;

/// 状态回调代理
@property (nonatomic, weak) id<IFImageBrowserDelegate> delegate;
...
/// 是否自动隐藏 id<IFIBImageData> 设置的映射视图，默认为 YES
...
@property (nonatomic, copy) NSArray<id<IFIBToolViewHandler>> *toolViewHandlers;
/// 默认工具视图处理器
@property (nonatomic, weak, readonly) IFIBToolViewHandler *defaultToolViewHandler;

/// Toast/Loading 处理器 (赋值可自定义)
@property (nonatomic, strong) id<IFIBAuxiliaryViewHandler> auxiliaryViewHandler;

/// 转场实现类 (赋值可自定义)
@property (nonatomic, strong) id<IFIBAnimatedTransition> animatedTransition;
/// 默认转场实现类 (可配置其属性)
@property (nonatomic, weak, readonly) IFIBAnimatedTransition *defaultAnimatedTransition;

/// 图片下载缓存相关的中介者（赋值可自定义）
@property (nonatomic, strong) id<IFIBWebImageMediator> webImageMediator;
