...
@implementation IFIBSheetAction
+ (instancetype)actionWithName:(NSString *)name action:(YBIBSheetActionBlock)action {
    IFIBSheetAction *sheetAction = [IFIBSheetAction new];
...
@interface IFIBSheetCell : UITableViewCell
@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, strong) CALayer *line;
@end
@implementation IFIBSheetCell
...
@interface IFIBSheetView () <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UITableView *tableView;
@end

@implementation IFIBSheetView {
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _cancelText = [IFIBCopywriter sharedCopywriter].cancel;
        _maxHeightScale = 0.7;
        _showDuration = 0.0;
        _hideDuration = 0.1;
        _cellHeight = 50;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    IFIBSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(IFIBSheetCell.self)];
    if (indexPath.section == 0) {
        cell.line.hidden = NO;
        IFIBSheetAction *action = self.actions[indexPath.row];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:IFIBSheetCell.self forCellReuseIdentifier:NSStringFromClass(IFIBSheetCell.self)];
    }
    return _tableView;
}
