...
@interface UIView ()
@property (nonatomic, strong, readonly) IFIBLoadingView *ybib_loading;
@end
...
- (void)ybib_addLoadingView {
    IFIBLoadingView *loading = self.ybib_loading;
...
- (void)setYbib_loading:(IFIBLoadingView *)ybib_loading {
    objc_setAssociatedObject(self, YBIBLoadingKey, ybib_loading, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (IFIBLoadingView *)ybib_loading {
    IFIBLoadingView *loading = objc_getAssociatedObject(self, YBIBLoadingKey);
    if (!loading) {
        loading = [IFIBLoadingView new];
...
@interface IFIBProgressDrawView : UIView
@property (nonatomic, assign) CGFloat progress;
@end
@implementation IFIBProgressDrawView
...
@interface IFIBLoadingView () {
    YBImageBrowserProgressType  _type;
}
@property (nonatomic, strong) UILabel *textLabel;
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, strong) IFIBProgressDrawView *drawView;
@property (nonatomic, copy) void(^clickTextLabelBlock)(void);
@end

@implementation IFIBLoadingView
...
- (IFIBProgressDrawView *)drawView {
    if (!_drawView) {
        _drawView = [IFIBProgressDrawView new];
...
- (UIImageView *)imageView {
    if (!_imageView) {
        ...
        _imageView.image = [IFIBIconManager sharedManager].loadingImage();
