...
@implementation IFIBImageData {
    __weak id _downloadToken;
    IFIBSentinel *_cuttingSentinel;
...
- (void)initValue {
    _defaultLayout = _layout = [IFIBImageLayout new];
    _loadingStatus = YBIBImageLoadingStatusNone;
    _compressingSize = 4096 * 4096;
    _shouldPreDecodeAsync = YES;
    _freezing = NO;
    _cuttingSentinel = [IFIBSentinel new];
    _interactionProfile = [IFIBInteractionProfile new];
...
- (void)loadYBImage {
    ...
    __block IFImage *image;
    __weak typeof(self) wSelf = self;
    void(^dealBlock)(void) = ^{
        if (name.length > 0) {
            image = [IFImage imageNamed:name decodeDecision:decision];
        } else if (path.length > 0) {
            image = [IFImage imageWithContentsOfFile:path decodeDecision:decision];
        } else if (data.length > 0) {
            image = [IFImage imageWithData:data scale:UIScreen.mainScreen.scale decodeDecision:decision];
        }
        YBIB_DISPATCH_ASYNC_MAIN(^{
            __strong typeof(wSelf) self = wSelf;
            if (!self) return;
...
- (void)loadPHAsset {
    ...
    YBIB_DISPATCH_ASYNC(YBIBImageProcessingQueue(), ^{
        [IFIBPhotoAlbumManager getImageDataWithPHAsset:self.imagePHAsset completion:^(NSData * _Nullable data) {
            if (self->_freezing) {
                self.loadingStatus = YBIBImageLoadingStatusNone;
                return;
            }
            IFImage *image = [IFImage imageWithData:data scale:UIScreen.mainScreen.scale decodeDecision:decision];
            __weak typeof(self) wSelf = self;
            YBIB_DISPATCH_ASYNC_MAIN(^{
                __strong typeof(wSelf) self = wSelf;
                if (!self) return;
...
- (void)saveToPhotoAlbumCompleteWithError:(nullable NSError *)error {
    if (error) {
        [self.yb_auxiliaryViewHandler() yb_showIncorrectToastWithContainer:self.yb_containerView text:[IFIBCopywriter sharedCopywriter].saveToPhotoAlbumFailed];
    } else {
        [self.yb_auxiliaryViewHandler() yb_showCorrectToastWithContainer:self.yb_containerView text:[IFIBCopywriter sharedCopywriter].saveToPhotoAlbumSuccess];
    }
}
...
- (void)yb_saveToPhotoAlbum {
    ...
    void(^unableToSave)(void) = ^(){
        [self.yb_auxiliaryViewHandler() yb_showIncorrectToastWithContainer:self.yb_containerView text:[IFIBCopywriter sharedCopywriter].unableToSave];
    };
    ...
    [IFIBPhotoAlbumManager getPhotoAlbumAuthorizationSuccess:^{
        ...
        [self.yb_auxiliaryViewHandler() yb_showIncorrectToastWithContainer:self.yb_containerView text:[IFIBCopywriter sharedCopywriter].getPhotoAlbumAuthorizationFailed];
    }];
}
