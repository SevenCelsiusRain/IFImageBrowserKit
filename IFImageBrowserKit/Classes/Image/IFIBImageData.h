...
typedef NSURLRequest * _Nullable (^YBIBRequestModifierBlock)(IFIBImageData *imageData, NSURLRequest *request);

/// 根据图片逻辑像素和 scale 判断是否需要预解码的闭包
typedef BOOL (^YBIBPreDecodeDecisionBlock)(IFIBImageData *imageData, CGSize imageSize, CGFloat scale);

/// 修改 UIImage 并返回的闭包
typedef void (^YBIBImageModifierBlock)(IFIBImageData *imageData, UIImage *image, void(^completion)(UIImage *processedImage));

/// 单击事件的处理闭包
typedef void (^YBIBImageSingleTouchBlock)(IFIBImageData *imageData);

/// 内部图片滚动视图状态回调闭包
typedef void (^YBIBImageScrollViewStatusBlock)(IFIBImageData *imageData, UIScrollView *scrollView);


/**
 图片数据类，承担配置数据和处理数据的责任
 */
@interface IFIBImageData : NSObject <IFIBDataProtocol>
...
/// 手势交互动效配置文件
@property (nonatomic, strong) IFIBInteractionProfile *interactionProfile;
...
/// 默认图片布局类 (可配置其属性)
@property (nonatomic, weak, readonly) IFIBImageLayout *defaultLayout;
