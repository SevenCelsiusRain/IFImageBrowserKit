...
@interface IFIBImageCell () <YBIBImageDataDelegate, UIScrollViewDelegate, UIGestureRecognizerDelegate>
@end

@implementation IFIBImageCell {
...
- (void)prepareForReuse {
    ((IFIBImageData *)self.yb_cellData).delegate = nil;
...
- (void)setYb_cellData:(id<IFIBDataProtocol>)yb_cellData {
    ...
    ((IFIBImageData *)yb_cellData).delegate = self;
}
...
- (void)updateImageLayoutWithOrientation:(UIDeviceOrientation)orientation previousImageSize:(CGSize)previousImageSize {
    ...
    IFIBImageData *data = self.yb_cellData;
...
- (void)yb_imageIsInvalidForData:(IFIBImageData *)data {
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
    NSString *imageIsInvalid = [IFIBCopywriter sharedCopywriter].imageIsInvalid;
    if (self.imageScrollView.imageView.image) {
...
- (void)yb_imageDownloadFailedForData:(IFIBImageData *)data {
    if (self.imageScrollView.imageView.image) {
        [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
        [self.yb_auxiliaryViewHandler() yb_showIncorrectToastWithContainer:self text:[IFIBCopywriter sharedCopywriter].downloadFailed];
    } else {
        [self.yb_auxiliaryViewHandler() yb_showLoadingWithContainer:self text:[IFIBCopywriter sharedCopywriter].downloadFailed];
    }
}
...
- (void)respondsToPan:(UIPanGestureRecognizer *)pan {
    ...
    IFIBInteractionProfile *profile = ((IFIBImageData *)self.yb_cellData).interactionProfile;
...
- (IFIBImageScrollView *)imageScrollView {
    if (!_imageScrollView) {
        _imageScrollView = [IFIBImageScrollView new];
